...
def _get_coco_instances_meta_general(coco_path_list: list):
    thing_ids = []
    thing_classes = []
    thing_colors = []
    for item_coco_json in coco_path_list:
        coco = COCO(item_coco_json)
        cat_ids = coco.getCatIds()
        thing_ids += cat_ids
        infos_list = coco.loadCats(cat_ids)
        for item_info in infos_list:
            name = item_info['name']
            thing_classes.append(name)
    assert len(thing_classes) == len(thing_ids)
    assert len(thing_ids) == len(set(thing_ids))
    for index, _ in enumerate(thing_ids):
        index_coco_color = index % 80
        item_color = COCO_CATEGORIES[index_coco_color]['color']
        thing_colors.append(item_color)
...
def _get_coco_fewshot_instances_meta_general(coco_path_base, coco_path_novel):
    ret = _get_coco_instances_meta_general(coco_path_list = [coco_path_base, coco_path_novel])

    def get_coco_info(coco_path):
        coco = COCO(coco_path)
        cat_ids = coco.getCatIds()
        dataset_id_to_contiguous_id = {k: i for i, k in enumerate(cat_ids)}
        class_names = []
        cat_infos_list = coco.loadCats(cat_ids)
        for item_cat_info in cat_infos_list:
            name = item_cat_info['name']
            class_names.append(name)
        return dataset_id_to_contiguous_id, class_names

    base_dataset_id_to_contiguous_id, base_class_names = get_coco_info(coco_path_base)
    novel_dataset_id_to_contiguous_id, novel_class_names = get_coco_info(coco_path_novel)
    ret['novel_dataset_id_to_contiguous_id'] = novel_dataset_id_to_contiguous_id
    ret['novel_classes'] = novel_class_names
    ret['base_dataset_id_to_contiguous_id'] = base_dataset_id_to_contiguous_id
    ret['base_classes'] = base_class_names
    return ret
