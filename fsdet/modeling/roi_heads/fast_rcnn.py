...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
            self, cfg, input_size, num_classes, cls_agnostic_bbox_reg, box_dim = 4, with_box_reg = True
    ):
        """
        Args:
            cfg: config
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class FastRCNNOutputLayers_WithFC(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(FastRCNNOutputLayers_WithFC, self).__init__()
        ...
        # background class
        # (hence + 1)
        self.independent_FC = nn.Linear(input_size, input_size)
        self.cls_score = nn.Linear(input_size, num_classes + 1)
        nn.init.normal_(self.cls_score.weight, std = 0.01)
        nn.init.normal_(self.independent_FC.weight, std = 0.01)
    ...
    def forward(self, x):
        if x.dim() > 2:
            x = torch.flatten(x, start_dim = 1)
        x = F.relu(self.independent_FC(x))
        scores = self.cls_score(x)
        if (self.with_box_reg):
            proposal_deltas = self.bbox_pred(x)
        else:
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class FastRCNNOutputLayers_WithDoubleFC(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(FastRCNNOutputLayers_WithDoubleFC, self).__init__()
        ...
        # background class
        # (hence + 1)
        self.independent_FC = nn.Sequential(
        ...
        self.cls_score = nn.Linear(input_size, num_classes + 1)
        nn.init.normal_(self.cls_score.weight, std = 0.01)
        nn.init.normal_(self.independent_FC[0].weight, std = 0.01)
        nn.init.normal_(self.independent_FC[2].weight, std = 0.01)
...
@ROI_HEADS_OUTPUT_REGISTRY.register()
class CosineSimOutputLayers_WithFC(nn.Module):
    """
    Two outputs
    (1) proposal-to-detection box regression deltas (the same as
        the FastRCNNOutputLayers)
    ...
    def __init__(
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(CosineSimOutputLayers_WithFC, self).__init__()
        self.with_box_reg = with_box_reg
...
class FastRCNNOutputs_Hierarchical(object):
    """
    A class that stores information about outputs of a Fast R-CNN head.
    """
    ...
    def __init__(
        ...
        if (gt_classes is not None):
            self.gt_classes = torch.tensor(gt_classes).long().cuda()
    ...
    def losses(self):
        ...
        Returns:
            A dict of losses (scalar tensors) containing keys "loss_cls" and "loss_box_reg".
        """
        loss_root_cls = self.softmax_cross_entropy_loss()
        loss_root_box_reg = self.smooth_l1_loss()
        return loss_root_cls, loss_root_box_reg
        # return {
        #     "root_loss_cls": self.softmax_cross_entropy_loss(),
        #     "root_loss_box_reg": self.smooth_l1_loss(),
        # }
...
class Hierarchical_Post_Process():
    def __init__(self, box2box_transform, scores_record, res_class_id, box_deltas_record, proposals, image_size):
        self.box2box_transform = box2box_transform
        self.scores_record = scores_record
        self.res_class_id = res_class_id
        self.box_deltas_record = box_deltas_record
        self.image_shape = image_size
        self.proposals = torch.stack(proposals)
    ...
    def predict_boxes(self):
        ...
        assert len(self.proposals) == len(self.scores_record)
        assert self.box_deltas_record.shape == self.proposals.shape
        boxes = self.box2box_transform.apply_deltas(
    ...
    def inference(self, score_thresh, nms_thresh, topk_per_image):
        boxes = self.predict_boxes()
        scores_record = self.scores_record

        # Convert to Boxes to use the `clip` function ...
        boxes = Boxes(boxes.reshape(-1, 4))
        boxes.clip(self.image_shape)
        boxes = boxes.tensor
        # boxes = boxes.tensor.view(-1, 1, 4)  # R x C x 4
        class_ids = self.res_class_id
        # Filter results based on detection scores
        filter_mask = scores_record > score_thresh  # R x K [1000,287]
        # R' x 2. First column contains indices of the R predictions;
        # Second column contains indices of classes.
        # filter_inds = filter_mask.nonzero()
        # if num_bbox_reg_classes == 1:
        ...
        # else:
        #     boxes = boxes[filter_mask]  # [1000, 287, 4]
        scores = scores_record[filter_mask]  # [1000, 287]
        class_ids = class_ids[filter_mask]
        boxes = boxes[filter_mask]

        # Apply per-class NMS
        keep = batched_nms(boxes, scores, class_ids, nms_thresh)
        if topk_per_image >= 0:
            keep = keep[:topk_per_image]
        boxes, scores, class_ids = boxes[keep], scores[keep], class_ids[keep]

        result = Instances(self.image_shape)
        result.pred_boxes = Boxes(boxes)
        result.scores = scores
        result.pred_classes = class_ids
        return result
